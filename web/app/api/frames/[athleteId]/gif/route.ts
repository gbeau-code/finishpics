 ...
 *
 * Generates an animated boomerang GIF from the athlete's IdentiLynx frames.
 * Requires a 'full' tier purchase.
 *
 * Boomerang sequence: [0,1,2,...,n-1, n-2,...,1] loops infinitely.
 * Frames are resized to ≤480px wide to keep file size reasonable.
 */
...
const GIF_MAX_WIDTH = 480   // resize frames to this width
...
export async function GET(
  ...
  try {
    ...
    for (let i = 0; i < athlete.frame_count; i++) {
      const path = frameUrl(athlete.frames_dir, i)
      rawFrames.push(await readImageBuffer(path))
    }
    ...
    // Build boomerang index sequence: [0,1,...,n-1, n-2,...,1]
    ...
    // Resize each frame and collect RGBA pixel data
